...
class SEBottleneck(nn.Module):
    expansion = 4
    def __init__(self, inplanes, planes, stride=1, cbam=None, downsample=None, reduction=16):
...
class ResNet(nn.Module):
    def __init__(self, block, layers, cbam=None, num_classes=1000):
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], cbam, stride=1)
    ...
    def get_features(self, layerNum=4, lastLayerBlockNum=-1):

        result = nn.Sequential(
            OrderedDict([('conv1', self.conv1),
                        ('bn1',self.bn1),
                        ('relu1',self.relu),
                        ('maxpool1',self.maxpool)]) )
        
        for layerIndex in range(layerNum):
            
            layerName = 'layer%d' % (layerIndex + 1)
            addLayer = getattr(self, layerName)
            if(layerIndex != layerNum - 1 or lastLayerBlockNum == -1):
                result.add_module(layerName, addLayer)
            else:
                result.add_module(layerName , addLayer[:lastLayerBlockNum])
...
if __name__ == '__main__':
    resnet34(pretrained=False).get_features(layerNum=3, lastLayerBlockNum=5)
