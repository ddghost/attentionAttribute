...
class MultiLabelDataset(data.Dataset):
    def __init__(self, root, label, transform = None, loader = default_loader):
        ...
        for line in labels:
            ...
            if os.path.exists(os.path.join(root, img_name)):
...
attr_nums['rap'] = 51
attr_nums['peta'] = 35
attr_nums['river'] = len(description['river'])


def Get_Dataset(experiment, rootPath):
    trainFileName = os.path.join(rootPath,'trainAttribute.txt')
    valFileName = os.path.join(rootPath,'valAttribute.txt')
    trainRoot = os.path.join(rootPath,'train')
    ...
    if experiment == 'pa100k':
        ...
        train_dataset = MultiLabelDataset(root=trainRoot,
                    label=trainFileName, transform=transform_train)
        val_dataset = MultiLabelDataset(root=valRoot,
                    label=valFileName, transform=transform_test)
        return train_dataset, val_dataset, attr_nums['river'], description['river']

if __name__ == '__main__':
    Get_Dataset('river','../../河流数据集/data(useful)/splitDataset_v2/splitDataset_k0')
